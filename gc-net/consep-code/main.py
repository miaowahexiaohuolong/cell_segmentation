...
def train():
    ...
    loss_fn = MultiLabelBCEDiceLoss(alpha=0.7, reduction='sum').to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.9)

    train_dataset = CoNSePDataset(root_path=Constants.DATA_ROOT, mode='train')
    ...
    for epoch in range(1, Constants.TOTAL_EPOCH + 1):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch}/{Constants.TOTAL_EPOCH}") as pbar:
            for img, inst_mask, type_mask in pbar:
                ...
                type_mask = torch.clamp(type_mask, 0, 1)

                optimizer.zero_grad()
                pred = model(img)  # 输出: [B, C, H, W]

                # 扩展掩码以匹配预测的通道数
                inst_mask = inst_mask.repeat(1, pred.size(1), 1, 1)
                type_mask = type_mask.repeat(1, pred.size(1), 1, 1)

                # 分别计算实例掩码和类别掩码的损失
                loss_inst = loss_fn(pred, inst_mask)
                loss_type = loss_fn(pred, type_mask)
                loss = 0.5 * loss_inst + 0.5 * loss_type  # 简单加权求和，可根据实际调整权重
