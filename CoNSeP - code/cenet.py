...
class CE_Net_(nn.Module):
    def __init__(self, num_classes=Constants.BINARY_CLASS, num_channels=3):
        ...
        # 1. 创建 ResNet34 骨架（不加载在线预训练权重）
        resnet = models.resnet34(pretrained=False)  # 关键：pretrained=False
        
        # 2. 加载本地预训练权重（官方 .pth 格式，纯权重文件）
        local_weight_path = 'resnet34-333f7ec4.pth'  # 本地权重路径（需存在）
        # 对于纯权重文件（.pth），无需特殊参数（默认 weights_only=True 兼容）
        state_dict = torch.load(local_weight_path,weights_only=False)  # 加载到 CPU/GPU
        resnet.load_state_dict(state_dict)  # 加载权重到 ResNet34 骨架
